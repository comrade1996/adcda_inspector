...
class _HomeScreenState extends State<HomeScreen> {
  ...
  final UAEPassService _uaePassService = Get.find<UAEPassService>();
  ...
  Future<void> _refreshUserProfile() async {
    ...
    if (authMethod == 'uae_pass') {
      final uaePassUniqueName = await secureStorage.read(
        key: 'uae_pass_unique_name',
      );
      if (uaePassUniqueName != null &&
          uaePassUniqueName.isNotEmpty &&
          _userProfile != null &&
          (_userProfile!.uniqueName == null ||
              _userProfile!.uniqueName!.isEmpty)) {
        final updatedProfile = UserProfile(
          id: _userProfile!.id,
          userName: _userProfile!.userName,
          name: _userProfile!.name,
  ...
  Future<void> _loadLanguagePreference() async {
    try {
      ...
      final storedLanguageId =
          prefs.getInt('languageId') ?? AppConstants.defaultLanguageId;
    ...
    try {
      ...
      if (mounted) {
        setState(() {
          _surveys =
              surveys
                  .map((dto) => SurveyItem.fromSurveyDTO(dto, context))
                  .toList();
  ...
  void _startSurvey(int surveyId) {
    ...
    Future.delayed(const Duration(seconds: 2), () {
      ...
      Get.to(
        () => SurveyScreen(surveyId: surveyId, languageId: _currentLanguageId),
      );
    });
  }
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          ...
          LanguageSelector(onLanguageChanged: _handleLanguageChanged),
        ],
      ),
      ...
      body: Container(
        ...
        decoration: BoxDecoration(
          gradient: LinearGradient(
            ...
            colors: [Color(0xFF0A0A0A), Color(0xFF101010), Color(0xFF1A1A1A)],
          ),
        ),
        child: SafeArea(
          child: Column(
  ...
  Widget _buildLoadingView() {
    return Center(
      child: Column(
        ...
        children: [
          Lottie.asset('assets/images/loader.json', width: 100, height: 100),
          SizedBox(height: 16),
          Text(
            ...
            accountName: FutureBuilder<String?>(
              ...
              builder: (context, snapshot) {
                ...
                // Otherwise fall back to user profile data
                  ...
                  if (_userProfile!.uniqueName != null &&
                      _userProfile!.uniqueName!.isNotEmpty) {
          ...
          ListTile(
            ...
            onTap: () async {
              ...
              if (confirm) {
                ...
                // Check if user is logged in with UAE Pass
                final isUaePassUser =
                    await _authService.isUaePassAuthenticated();
                print(
                  '👤 Logout: User is UAE Pass authenticated: $isUaePassUser',
                );

                if (isUaePassUser) {
                  // Use UAE Pass logout flow if user is authenticated with UAE Pass
                  print('🔑 Calling UAE Pass logout method');
                  if (context.mounted) {
                    await _uaePassService.logout(context);
                    await _authService.logout();
                  }
                } else {
                  // Use standard logout for non-UAE Pass users
                  print('🔑 Calling standard logout method');
                  await _authService.logout();
                }
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        return Container(
            ...
            .animate()
            .fadeIn(duration: 300.ms, delay: (100 + index * 100).ms)
